...
def analyze(mode_key_path):
    ...
    mode_types = list(set([accelerometers.mode_map(mode, mode_key_path) for mode in combined_bycat_log.columns]))

    for t in mode_types:
        modes = accelerometers.mode_match(combined_bycat_log.columns, t, mode_key_path)
    ...
    combined_bymode_type = combined_bycat_log.rename(lambda x: accelerometers.mode_map(x, mode_key_path), axis=1).groupby('Category', axis=1).mean()
...
if __name__ == '__main__':
    ...
    parser.add_option("-m", "--modekey_file", dest="mfile", help="Aggregation Filename pointing to csv with dictionary of mode types", default=accelerometers.MODE_KEY_PATH)
